...
- GitHubへコードがアップされるたびに自動でビルド・テスト（CI）が走る。GitHub上でマージが完了すると、自動でインフラ側へデプロイされる（CD）
...
- CIはテストコードがないとできない、プッシュされたタイミングでCIが走る
- 自分のリポジトリではCIでテストが落ちても権限が最高権限なのでマージできるが、そうでない場合はテストが通らないとマージできない可能性が高い

- CDでやっていることはローカル環境でやっていたことと同じ、ただクラウド上で実行しているという違い
